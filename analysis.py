...
print(f"Среднее время (старый интерфейс): {group_old['Время'].mean():.1f} сек")
print(f"Среднее время (новый интерфейс): {group_new['Время'].mean():.1f} сек")
...
print(f"Медиана ошибок (старый интерфейс): {group_old['Ошибки'].median()}")
print(f"Медиана ошибок (новый интерфейс): {group_new['Ошибки'].median()}")

#3. Практическая значимость:
#Рассчет велечин эффекта через коэффициент r:
n1 = len(group_old)
n2 = len(group_new)
r_effect = 866.0 / (n1 * n2)
print(f"r = {r_effect:.2f}")  # Например, r = 0.4 (средний эффект)
